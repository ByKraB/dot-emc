...
(function() {
    ...
    Defines = (function() {
        ...
        Defines.prototype.replaceVars = function(template,vars){

            var varname = vars.templateSettings.varname;
            var includeRegexp = /#def\.include\([^\)]*/gim;
            var replaceRegexp = new RegExp(varname+"\.[A-Za-z1-9]*","gim");

            var includes = template.match(includeRegexp);
            if(!includes)
                return template;


            includes.map(function(include) {
                /* еще одна рекурсия(формально для одного вхождения it в #def), но в случае если два и более вхождений it в блок #def, без нее не обойтись*/
                var replaceParts = include.match(replaceRegexp);
                if(!replaceParts)
                    return;

                var includeValue = include;
                replaceParts.map(function(item) {
                    var replaceValue = vars[item.replace(varname+'.', '')] || '';
                    includeValue = includeValue.replace(item, '\'' + replaceValue + '\'')
                });
                template = template.replace(include, includeValue);
            });
